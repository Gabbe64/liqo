...
| networking.denyDirectConnections | bool | `false` | Never use direct connections between provider clusters. When enabled, Services always route cross-provider traffic through the consumer cluster (indirect endpoints), even for Services annotated with liqo.io/use-direct-connections. |
