...
| networking.clientResources | list | `[{"apiVersion":"networking.liqo.io/v1beta1","resource":"wggatewayclients"},{"apiVersion":"networking.liqo.io/v1beta1","resource":"genevegatewayclients"}]` | Set the list of resources that implement the GatewayClient |
...
| networking.gatewayTemplates.geneve.client.service.annotations | object | `{}` | Annotations for the gateway client service. |
| networking.gatewayTemplates.geneve.client.service.port | int | `31840` | Port used to expose the gateway client. With the NodePort service type this value is also pinned as the node port, so it must fall inside the cluster's --service-node-port-range (30000-32767 by default): a Geneve device uses one UDP port for both listening and transmitting and the kernel refuses to change it, so a translated node port would be unreachable. The gateway server must be given the same value through the flags --gw-server-service-port and --gw-server-service-nodeport, and the gateway refuses to start if they disagree. |
| networking.gatewayTemplates.geneve.client.service.type | string | `"NodePort"` | Service type used to expose the gateway client. The Geneve server replies to a configured endpoint rather than to the observed source, so the client must be reachable and always needs a service. Only NodePort and LoadBalancer are supported: the service carries externalTrafficPolicy=Local, which the API server rejects on a ClusterIP service. |
...
| networking.serverResources | list | `[{"apiVersion":"networking.liqo.io/v1beta1","resource":"wggatewayservers"},{"apiVersion":"networking.liqo.io/v1beta1","resource":"genevegatewayservers"}]` | Set the list of resources that implement the GatewayServer |
