...
| common.extraArgs | list | `[]` | Extra arguments for all liqo pods, excluding virtual kubelet and gateway pods. |
...
| common.nodeSelector | object | `{}` | NodeSelector for all liqo pods, excluding virtual kubelet, gateway and fabric pods. |
| common.tolerations | list | `[]` | Tolerations for all liqo pods, excluding virtual kubelet, gateway and fabric pods. |
...
| networking.clientResources | list | `[{"apiVersion":"networking.liqo.io/v1beta1","resource":"wggatewayclients"},{"apiVersion":"networking.liqo.io/v1beta1","resource":"vxlangatewayclients"},{"apiVersion":"networking.liqo.io/v1beta1","resource":"genevegatewayclients"}]` | Set the list of resources that implement the GatewayClient |
...
| networking.fabric.config.routeMonitor | bool | `true` | Enable/Disable the route monitor for the fabric pod. It means that the fabric pod will monitor the routing rules and will restore them in case of changes. |
...
| networking.gatewayTemplates | object | `{"container":{"gateway":{"image":{"name":"ghcr.io/liqotech/gateway","version":""}},"geneve":{"image":{"name":"ghcr.io/liqotech/gateway/geneve","version":""}},"geneveTunnel":{"image":{"name":"ghcr.io/liqotech/gateway/geneve-tunnel","version":""}},"vxlan":{"image":{"name":"ghcr.io/liqotech/gateway/vxlan","version":""}},"wireguard":{"image":{"name":"ghcr.io/liqotech/gateway/wireguard","version":""}}},"geneve":{"client":{"service":{"port":51842,"type":"NodePort"}},"vni":100},"nftablesMonitor":true,"ping":{"interval":"2s","lossThreshold":5,"updateStatusInterval":"10s"},"pod":{"priorityClassName":"","tolerations":[]},"replicas":1,"routeMonitor":true,"server":{"service":{"allocateLoadBalancerNodePorts":"","annotations":{}}},"vxlan":{"static":{"client":{"service":{"port":51841,"type":"NodePort"}}},"vni":100},"wireguard":{"implementation":"kernel"}}` | Set the options for the default gateway (server/client) templates. The default templates use a WireGuard implementation to connect the gateway of the clusters. These options are used to configure only the default templates and should not be considered if a custom template is used. |
...
| networking.gatewayTemplates.container.geneveTunnel.image.name | string | `"ghcr.io/liqotech/gateway/geneve-tunnel"` | Image repository for the geneve external-tunnel container. Distinct from the "geneve" container above, which terminates the internal fabric inside the same pod. |
| networking.gatewayTemplates.container.geneveTunnel.image.version | string | `""` | Custom version for the geneve-tunnel image. If not specified, the global tag is used. |
| networking.gatewayTemplates.container.vxlan.image.name | string | `"ghcr.io/liqotech/gateway/vxlan"` | Image repository for the vxlan container. |
| networking.gatewayTemplates.container.vxlan.image.version | string | `""` | Custom version for the vxlan image. If not specified, the global tag is used. |
...
| networking.gatewayTemplates.geneve.client.service.port | int | `51842` | Port used to expose the gateway client. |
| networking.gatewayTemplates.geneve.client.service.type | string | `"NodePort"` | Service type used to expose the gateway client. The Geneve server replies to a configured endpoint rather than to the observed source, so the client must be reachable and always needs a service. |
| networking.gatewayTemplates.geneve.vni | int | `100` | Set the Virtual Network Identifier used by the Geneve gateway tunnels. It must match on both peered clusters: nothing validates it, and a mismatch silently black-holes the tunnel. |
| networking.gatewayTemplates.nftablesMonitor | bool | `true` | Enable/Disable the nftables monitor for the gateway pods. It means that the gateway pods will monitor the nftables rules and will restore them in case of changes. |
...
| networking.gatewayTemplates.pod.priorityClassName | string | `""` | PriorityClassName (https://kubernetes.io/docs/concepts/scheduling-eviction/pod-priority-preemption/#pod-priority) for the gateway pods. |
| networking.gatewayTemplates.pod.tolerations | list | `[]` | Additional tolerations for gateway pods |
| networking.gatewayTemplates.replicas | int | `1` | Set the number of replicas for the gateway deployments |
| networking.gatewayTemplates.routeMonitor | bool | `true` | Enable/Disable the route monitor for the gateway pods. It means that the gateway pods will monitor the routing rules and will restore them in case of changes. |
...
| networking.gatewayTemplates.vxlan.static | object | `{"client":{"service":{"port":51841,"type":"NodePort"}}}` | Options for the VXLAN templates using the "static" tunnel mode. In this mode both gateways are configured with each other's endpoint instead of learning it from the data plane: this allows the outer UDP source port to be hashed (so RSS and ECMP can spread the tunnel traffic), but requires the two gateways to be mutually reachable. Use the default "nat-traversal" templates when the client is behind NAT. |
| networking.gatewayTemplates.vxlan.static.client.service.port | int | `51841` | Port used to expose the gateway client in "static" tunnel mode. |
| networking.gatewayTemplates.vxlan.static.client.service.type | string | `"NodePort"` | Service type used to expose the gateway client, which in "static" tunnel mode must be reachable by the server. |
| networking.gatewayTemplates.vxlan.vni | int | `100` | Set the VXLAN Network Identifier used by the VXLAN gateway tunnels. It must match on both peered clusters. |
...
| networking.serverResources | list | `[{"apiVersion":"networking.liqo.io/v1beta1","resource":"wggatewayservers"},{"apiVersion":"networking.liqo.io/v1beta1","resource":"vxlangatewayservers"},{"apiVersion":"networking.liqo.io/v1beta1","resource":"genevegatewayservers"}]` | Set the list of resources that implement the GatewayServer |
