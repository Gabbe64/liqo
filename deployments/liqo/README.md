...
| controllerManager.config.denyDirectConnections | bool | `false` | Prevent endpoints reachable only through a direct provider-to-provider connection from being added to the reflected EndpointSlice. When enabled, the shadowendpointslice controller drops the endpoints referring to pods hosted on other providers (listed in the direct-connection annotation data). |
...
| ipam.externalCIDR | string | `""` | The IP subnet used for the external CIDR. If empty, a free network will be automatically allocated by the IPAM. If set, the IPAM will try to allocate the exact network, failing in case of conflicts. Set it only if you know what you are doing. |
...
| ipam.internalCIDR | string | `""` | The IP subnet used for the internal CIDR. These IPs are assigned to the Liqo internal-network interfaces. If empty, a free network will be automatically allocated by the IPAM. If set, the IPAM will try to allocate the exact network, failing in case of conflicts. Set it only if you know what you are doing. |
| ipam.podCIDR | string | `""` | The IP subnet used by the pods in your cluster, in CIDR notation (e.g., 10.0.0.0/16). Deprecated: used as fallback if podCIDRs is empty. |
| ipam.podCIDRs | list | `[]` | The IP subnets used by the pods in your cluster, in CIDR notation. (e.g., 10.0.0.0/16, 10.88.0/16). |
...
| ipam.serviceCIDR | string | `""` | The IP subnet used by the services in you cluster, in CIDR notation (e.g., 172.16.0.0/16). |
...
| networking.fabric.config.routeConfigurationRulePriority | int | `0` | The priority used by the liqo-controller-manager to set the routing rules on the fabric (hostnetwork). It can be useful to solve race conditions between CNI daemonsets and liqo-fabric. For example the AWS CNI should add some rules with a fixed lower priority (lowest comes first) which breaks Liqo networking. Setting an explicit priority on Liqo's fabric routing rules allows them to be correctly ordered with respect to CNI rules. We suggest using a value between 101 and 500. If the value is 0, no priority is set, and Linux will automatically select a lower priority than the existing rules. |
