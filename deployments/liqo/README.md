...
| controllerManager.config.denyDirectConnections | bool | `false` | Prevents the usage of direct connections by provider clusters.  When enabled, the provider cluster will not route traffic directed to another provider through their direct connection.  |
