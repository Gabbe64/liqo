...
// Data is a struct representing the addresses of the pods deployed on a remote cluster
...
// Index is an in-memory lookup table from IP to clusterID.
//
...
func (c *Data) Add(clusterID string, ips ...string) {
	if c.ByCluster == nil {
...
func (c *Data) BuildIndex() *Index {
	if c == nil || len(c.ByCluster) == 0 {
		return nil
	}

	index := &Index{IPToCluster: make(map[string]string)}
...
func (i *Index) LookupClusterID(ip string) (clusterID string, found bool) {
...
func (c *Data) ToJSON() ([]byte, error) {
	return json.Marshal(c)
}

// FromJSON deserializes JSON data into the DirectConnectionData.
func (c *Data) FromJSON(data []byte) error {
