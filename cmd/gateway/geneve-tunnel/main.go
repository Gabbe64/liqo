...
// Package main is the entrypoint of the Geneve tunnel runtime.
package main
...
import (
	...
	"github.com/liqotech/liqo/pkg/gateway/tunnel/geneve"
...
func run(cmd *cobra.Command, _ []string) error {
	...
	if err := geneve.ValidateOptions(options); err != nil {
		return err
	}

	// Setup the Geneve tunnel interface.
	linkIndex, err := geneve.InitGeneveLink(cmd.Context(), options)
	if err != nil {
		return fmt.Errorf("unable to init Geneve link: %w", err)
	}
	...
	if err := mgr.Add(manager.RunnableFunc(func(ctx context.Context) error {
		return geneve.EnsureConnection(ctx, mgr.GetClient(), mgr.GetScheme(), options)
	})); err != nil {
		return fmt.Errorf("unable to add connection enforcer: %w", err)
	}

	// The Geneve tunnel has no data-plane discovery: the kernel always hashes the
	// outer source port, so a server has no fixed port to reply to and cannot infer
	// its peer from received traffic. The server therefore takes the peer endpoint
	// from the PeerEndpoint resource, which arrives once the client has published
	// it. The client is configured with the server's endpoint at creation time.
	if options.GwOptions.Mode == gateway.ModeServer {
		per := geneve.NewPeerEndpointReconciler(mgr.GetClient(), mgr.GetScheme(),
			mgr.GetEventRecorderFor("geneve-peer-endpoint-controller"), options, linkIndex)
		if err := per.SetupWithManager(mgr); err != nil {
			return fmt.Errorf("unable to setup peer endpoint reconciler: %w", err)
		}
	}
	if options.GwOptions.Mode == gateway.ModeClient && geneve.IsDNSRoutineRequired(options) {
		if err := mgr.Add(manager.RunnableFunc(func(ctx context.Context) error {
			return geneve.RunDNSRoutine(ctx, options, linkIndex)
		})); err != nil {
